...
class SetupGameWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.game_date_picker.setDate(QDate.currentDate())  # Set the default date to today's date
        self.game_date_picker.setCalendarPopup(True)  # Show a calendar popup
        self.game_date_picker.setDisplayFormat("MMMM d, yyyy")  # Set the display format to "March 24, 2024"
        section_layout.addWidget(game_date_label)
        section_layout.addWidget(self.game_date_picker)

        game_location_label = QLabel("Game Location")
        section_layout.addWidget(game_location_label)
        section_layout.addWidget(self.game_location_edit)
